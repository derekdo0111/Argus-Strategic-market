...
│  Q(商模+护城河+资本运作) + R1(外部环境) + R2(管理层+人才) │
│  + R3(控股结构) + V(估值概述)                          │
...
│           重大事件(定增/并购/重组/合同/诉讼) ★v0.7.3    │
│  输出: structured_facts (注入QRV Agent数据包)          │
└──────────────┬──────────────────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────────────────┐
│  Step 8: QRV Agent v4 (取代原Step 8+9)                │
...
6. **FCF分红覆盖率** — `count(FCF ≥ dividend_paid_cf, 5y) >= 4` (v0.7.0 新增)
7. **供应商挤压** — `净供应商欠款/营业成本CAGR − 营收CAGR < 10pp` (v0.7.0 新增)
8. **有息负债率趋势** — `有息负债/总资产 3年变化 < 10pp` (v0.7.0 新增)
...
## Step 4: 现金质量门 [软门] <sup>v0.3.0</sup> <sup>v0.7.0: 8维度</sup>

### 判定标准
8子维度判定通过/未通过。**不淘汰，仅标记**。
...
| 经营CF CV | < 0.5 | v0.2.x |
| FCF分红覆盖 | ≥ 4/5 | **v0.7.0** |
| 供应商挤压 | 增速差 < 10pp | **v0.7.0** |
| 有息负债趋势 | 3年变化 < 10pp | **v0.7.0** |

> **v0.7.2**: dim7 供应商挤压新增 reason 字段，区分三种情况：
> - `not_applicable_no_supplier_credit` — 无实物供应商（广告/软件/轻资产），净欠款连续为0，不适用
> - `insufficient_data_for_cagr` — 数据不足无法计算CAGR
> - 正常计算 → 输出 supplier_ratio_cagr / revenue_cagr / squeeze_gap
...
**硬排除（不入股池）:**
- 可支配现金均值 ≤ 0 → **直接排除**，不进入股池。说明公司过去5年无法产生可供股东分配的现金流。
...
- **搜索模块**: 5次搜索覆盖 Q/R/V 三维度
  1. `q_websearch` — 商业模式 + 护城河 + 收入结构 + **资本运作** (v0.7.3新增)
  2. `r1_websearch` — 外部环境与宏观风险
  3. `r2_websearch` — 管理层 + 公司治理 + **人才结构** (新增)
  4. `r3_websearch` — 控股结构与关联交易
  5. `v_websearch` — 估值概述
- **置信度标注**: HIGH/MEDIUM/LOW/NONE，已注入 reference_index 供 LLM 在参考来源表中展示
...
- **功能**: 从 websearch snippets 预提取结构化事实
  ...
  8. corporate_events — 定增/并购/重组/重大合同/诉讼 ★v0.7.3
...
- **分析框架**: Q(质量) + R(韧性) + V(估值) 三维度 11模块，每模块**强制输出量化表格**
  ...
  - R3: 控股结构 → 股权结构表 + 关联交易
  - R4: 重大事件与资本运作 → 事件清单表 + 影响分析 + 跨维度联动 ★v0.7.3
  - V1: 价值陷阱筛查 → CQ 8维度表 + 资产负债快照
...
- **v0.7.6 输出质量**: v0.7.5全部 + 置信度升级为质量加权(来源·密度·时效三维打分) + 前端锚点跳转修复(引用可点击 + sanitize允许id + TOC子项导航正确) + 置信度使用规则(低置信度来源降权)
